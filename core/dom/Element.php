...
class Element extends DOMElement {
	 ...
	 */
	public function matches($selectors){
	 ...
	 * @return void
	 */
	public function prop_set_className($value){
	...
	public function prop_get_innerHTML(){
		$childHtmlArray = [];
	...
	public function prop_set_innerHTML($html){
		...
		if(function_exists('mb_convert_encoding')) {
			...
			$importDocument->loadHTML('<meta http-equiv="content-type" content="text/html; charset=utf-8">'.$htmlWrapped);
		}
	...
	public function prop_get_outerHTML(){
		return $this->ownerDocument->saveHTML($this);
	}

	public function prop_set_outerHTML($html){
		$fragment = $this->ownerDocument->createDocumentFragment();
		$fragment->appendXML($html);
		$this->replaceWith($fragment);
	}

	public function prop_get_innerText(){
		return $this->textContent;
	}

	public function prop_set_innerText($value){
		$this->textContent = $value;

		return $this->textContent;
	}

	public function prop_get_dataset(){
	...
	public function prop_get_checked(){
		return $this->hasAttribute("checked");
	}

	public function prop_set_checked($checked){
		if($checked) {
...
// TODO: Use `form` attribute when implemented: https://github.com/PhpGt/Dom/issues/161
	...
	public function prop_get_selected(){
		return $this->hasAttribute("selected");
	}

	public function prop_set_selected($selected){
		if($selected) {
	...
	private function value_get_select(){
		...
		foreach($options as $option) {
			if($this->isSelectOptionSelected($option)) {
				...
				$value = $value ? $value : trim($option->innerText);
				break;
			}
		}
	...
	private function value_set_input($newValue){
